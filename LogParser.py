...
HOST = 'localhost' #'host'
USER = 'root' #'db_user'
PASSWORD = 'ofaTeam' #'password'
DB_NAME = 'ips' #'database_name'
...
def get_source(attributes):
	...
	interface = attributes[3].split('=')[1].strip()
	interface = interface.replace('"', '') # remove quotes
	return ip, interface
...
def get_destination(attributes):
	...
	interface = attributes[6].split('=')[1].strip()
	interface = interface.replace('"', '') # remove quotes
...
def insert_traffic_into_db(cursor, source_ip, dest_ip, port):
	# Create select query	
	select_query = 'SELECT * from traffic'
	select_query += ' WHERE srcIp = "' + source_ip
	select_query += '" AND dstIp = "' + dest_ip
	select_query += '" AND port = ' + str(port) 
	...
	if cursor.rowcount == 0:
		...
		insert_query = 'INSERT INTO traffic '
		insert_query += '(srcip, dstip, port) VALUES('
		insert_query += '"' + source_ip + '","' + dest_ip
		insert_query += '",' + str(port) + ')'
...
def update_log_count(cursor, log_id, date, time_interval):
	...
	select_query += ' WHERE log_id = "' + str(log_id)
	select_query += '" AND date = "' + str(date) + '"'
	...
	if cursor.rowcount == 0:
		...
		insert_query += '(log_id, date, '+ time_interval
		insert_query += ') VALUES(' + str(log_id) + ', "'
		insert_query += str(date) + '", 1)'

		# Execute created query
		cursor.execute(insert_query)
	else:
		update_query = 'UPDATE log SET ' + time_interval
		update_query += '=' + time_interval + '+1 '
		update_query += 'WHERE log_id = ' + str(log_id)
		update_query += ' AND date = "' + str(date) + '"'
